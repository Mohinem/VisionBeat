"""MediaPipe pose backend that normalizes landmarks into TrackerOutput."""
...
class MediaPipePoseProvider(PoseProvider):
    """Wrapper around MediaPipe Pose that emits project-native landmarks."""
    ...
    def _load_tasks_pose_factory(import_failures: list[str]) -> Any | None:
        ...
        try:
            mediapipe = sys.modules.get("mediapipe") or import_module("mediapipe")
            tasks_python = (
                sys.modules.get("mediapipe.tasks.python")
                or import_module("mediapipe.tasks.python")
            )
            vision = (
                sys.modules.get("mediapipe.tasks.python.vision")
                or import_module("mediapipe.tasks.python.vision")
            )
        except ImportError as exc:
        ...
        model_asset_path = MediaPipePoseProvider._ensure_pose_model_asset()
    ...
    def __post_init__(self) -> None:
        ...
        logger.info(
            "Initializing pose backend=%s complexity=%s detection_threshold=%.2f "
        ...
        if pose_factory is None:
            ...
            msg = (
                "Pose backend 'mediapipe' is unavailable. VisionBeat requires either the "
                "classic `mediapipe.solutions.pose.Pose` interface or the MediaPipe Tasks "
                "Pose Landmarker runtime.\n"
    ...
    def process(self, frame: Frame, timestamp: FrameTimestamp | float) -> TrackerOutput:
        ...
        if result.pose_landmarks is None:
            logger.debug(
                "Tracking backend=%s status=no_person_detected timestamp=%.3f",
                self.config.backend,
        ...
        logger.debug(
            "Tracking backend=%s status=%s timestamp=%.3f landmarks=%s",
            self.config.backend,
            status,
            frame_timestamp.seconds,
            sorted(landmarks),
...
class _TasksPoseAdapter:
    ...
    def process(self, frame: Any) -> SimpleNamespace:
